/**
 * Notification System for Student Pages
 */
...
(function() {
    ...
    const CONFIG = {
        refreshInterval: 60000, // 1 minute - more frequent for appointment cancellations
    };
    ...
    function renderNotifications(notifications) {
        ...
        notifications.forEach(function(notif) {
            ...
            // Determine notification type label and CSS class
            let typeLabel = '';
            let typeClass = '';
            if (notif.type === 'student_appointment_booked') {
                typeLabel = 'APPOINTMENT CONFIRMED';
                typeClass = 'appointment_booked'; // Green styling for confirmation
            } else if (notif.type === 'appointment_cancelled') {
                typeLabel = 'APPOINTMENT CANCELLED';
                typeClass = 'appointment_cancelled'; // Red styling for cancellation
            } else {
                typeLabel = notif.type.replace('_', ' ').toUpperCase();
    ...
    function handleNotificationClick(type, data) {
        // For appointment bookings and cancellations, navigate to appointments page
        if (type === 'student_appointment_booked' || type === 'appointment_cancelled') {
            window.location.href = 'appointment.php';
        }
    }
    
    window.StudentNotificationSystem = {
        init: init,
